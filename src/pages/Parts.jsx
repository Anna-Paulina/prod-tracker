...
export default function Parts({ parts, setParts, machines, machiningTimes, setMachiningTimes, setAssignments }) {
  ...
  const parseCSV = (text) => {
    ...
    const newAssignments = []; // one entry per ref+machine combo
    let count = 0;
    ...
    for (const line of dataLines) {
      ...
      // Add to assignments (qty=1, each piece is unitary)
      newAssignments.push({
    ...
    setParts([...newParts].map((ref) => ({ ref })));
    // Replace assignments with the freshly imported ones
  ...
  const removePart = (ref) => {
    ...
    setAssignments((prev) => prev.filter((a) => a.partRef !== ref));
  };
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p className="page-sub">Importez un CSV — les pièces sont automatiquement ajoutées au planning</p>
      </div>
      ...
      <div className="grid-2" style={{ marginBottom: 24 }}>
        ...
        <div className="card">
          ...
          {csvData && (
            <div style={{ background: "rgba(232,255,71,0.08)", border: "1px solid var(--accent)", borderRadius: 6, padding: "8px 12px", fontSize: 13, color: "var(--accent)", marginBottom: 8 }}>
              ✓ {csvData.count} ligne(s) importée(s) — planning mis à jour automatiquement
            </div>
          )}
          {csvError && (
          ...
          <div style={{ marginTop: 10, padding: "10px 12px", background: "var(--surface2)", borderRadius: 6, fontSize: 12, color: "var(--text-dim)" }}>
            ...
            <span style={{ marginTop: 6, display: "block" }}>⚡ Chaque pièce est considérée unitaire (qté = 1). Les pièces remplacent le planning courant à l'import.</span>
          </div>
        </div>
